 ...
 * response are intended for only one user and are not a valid response for requests by other users. A (non-shared) cache MAY cache the response.
 * Note: This usage of the word only controls where the response may be cached, and cannot ensure the privacy of the message content.
 * <p/>
 * no-cache
 ...
 * <p/>
 * s-maxage
 * If a response includes an s-maxage directive, then for a shared cache (but not for a cache), the maximum age specified by this directive overrides the maximum age specified by either the max-age directive or the Expires header. The s-maxage directive also implies the semantics of the proxy-revalidate directive (see section 14.9.4), i.e., that the shared cache must not use the entry after it becomes stale to respond to a subsequent request without first revalidating it with the origin server. The s- maxage directive is always ignored by a cache.
 ...
 * However, the choice of validator might affect performance. The best approach is for the intermediate cache to use its own validator when making its request. If the server replies with 304 (Not Modified), then the cache can return its now validated copy to the client with a 200 response..as(OK) If the server replies with a new entity and cache validator, however, the intermediate cache can compare the returned validator with the one provided in the client's request, using the strong comparison function. If the client's validator is equal to the origin server's, then the intermediate cache simply returns 304 (Not Modified). Otherwise, it returns the new entity with a 200 response..as(OK)
 ...
 * <p/>
 * For example, consider a hypothetical new response directive called community which acts as a modifier to the directive. We define this new directive to mean that, in addition to any non-shared cache, any cache which is shared only by members of the community named within its value may cache the response. An origin server wishing to allow the UCI community to use an otherwise response in their shared cache(s) could do so by including
 * <p/>
 * Cache-Control: private, community="UCI"
 * A cache seeing this header field will act correctly even if the cache does not understand the community cache-extension, since it will also see and understand the directive and thus default to the safe behavior.
 * <p/>
 ...
 */
 ...
 /**
 * The Content-MD5 entity-header field, as defined in RFC 1864 [23], is an MD5 digest of the entity-body for the purpose of providing an end-to-end message integrity check of.as(MIC) the entity-body. (Note: a MIC is good for detecting accidental modification of the entity-body in transit, but is not proof against malicious attacks.)
 * <p/>
 ...
 * range not satisfiable) response instead of a 200 response.as(OK) for
 ...
 * <p/>
 * 1. If the response status code is 100 or.as(Continue) 101 (Switching
 ...
 * 200 status.as(OK), or if the passed If-Modified-Since date is
 ...
 * bytes=500-999
 * - The 500 bytes (Byte offsets 9500-9999, inclusive):
 * bytes=-500
 * - Or bytes=9500-
 ...
 */
 ...
 /**
 * The Retry-After response-header field can be used with a 503 (Service Unavailable) response to indicate how Int64 the service is expected to be unavailable to the requesting client. This field MAY also be used with any 3xx response.as(Redirection) to indicate the minimum time the user-agent is asked wait before issuing the redirected request. The value of this field can be either an HTTP-date or an integer number of seconds (in decimal) after the time of the response.
 * <p/>
 ...
 */
 ...
 /**
 * The WWW-Authenticate response-header field MUST be included in 401 response.as(Unauthorized) messages. The field value consists of at least one challenge that indicates the authentication scheme(s) and parameters applicable to the Request-URI.
 * <p/>
 ...
 */
 ...
 String header = URLDecoder.decode(name().replace('$', '%'));
 ByteBuffer token = HttpMethod.UTF8.encode(header);
 ...
 */
 ...
 String key =
 HttpMethod.UTF8.decodeheaders.position(p1.as(ByteBuffer).limit(p2 - 1)).toString().trim();
 if (key.length() > 0) {
