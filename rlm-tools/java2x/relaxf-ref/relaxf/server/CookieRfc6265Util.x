 ...
 */
 ...
 do {
 while (input.hasRemaining() && Character.isWhitespace(input.mark().as(ByteBuffer).get()));
 Int begin = input.reset().position();
 while (input.hasRemaining() && '=' != input.mark().as(ByteBuffer).get());

 return ByteBuffer.allocate(
 input.reset(.as(ByteBuffer).flip().position(begin)).slice().limit()).put(input).array();
 ...
 */
 ...
 Byte b;
 while (input.hasRemaining() && !Character.isWhitespace(b = input.mark().as(ByteBuffer).get())
 && '=' != b);
 ...
 */
 ...
 if (!tok.hasRemaining()) {
 Boolean keep = false;
 while (input.hasRemaining() && ';' != (b = input.mark().as(ByteBuffer).get())
 ...
 */
 ...
 Serializable value = cookieRfc6265Util.value(byteBuffer.rewind().as(ByteBuffer));
 if (Null != value) {
 ...
 */
 ...
 while (buf.hasRemaining() && Character.isWhitespace(buf.mark().as(ByteBuffer).get()));
 Int keyBegin = buf.reset().position();

 while (buf.hasRemaining() && '=' != buf.mark().as(ByteBuffer).get());
 ByteBuffer ckey = buf.duplicate(.as(ByteBuffer).reset().flip().position(keyBegin)).slice();
 while (buf.hasRemaining() && Character.isWhitespace(buf.mark().as(ByteBuffer).get()));
 Int vBegin = buf.reset().position();

 while (buf.hasRemaining()) {
 switch (buf.mark().as(ByteBuffer).get()) {
 case ';':
 case '\r':
 case '\n':
 break;
 ...
 ret =
 new Pair<Pair<Buffer, ByteBuffer>, Pair>(new Pair(ckey.reset(), buf.duplicate(.as(ByteBuffer).reset().flip()
 ...
 ret =
 new Pair<Pair<ByteBuffer, ByteBuffer>, Pair>(new Pair(ckey, buf.duplicate(.as(ByteBuffer).reset().flip().position(vBegin))
 .slice()), ret);
 }
